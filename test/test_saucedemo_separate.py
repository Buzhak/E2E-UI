...
def test_login(page: Page):
    ...
    page.get_by_placeholder('Username').fill(
        os.getenv('login', 'standard_user')
    )
    page.get_by_placeholder('Password').fill(
        os.getenv('password', 'secret_sauce')
    )
    page.get_by_text('Login').click()

    # Проверяем что логин прошёл успешно и есть список товаров на странице
    expect(
        page.locator('.inventory_container'), LOGIN_FAILED_MESSAGE
    ).to_be_visible()
...
def test_add_somesing_to_cart(browser: Browser):
    ...
    expect(
        first_item.get_by_role('button', name='Remove'), CART_EMPTY
    ).to_be_visible()
...
def test_checkout_card(browser: Browser):
    ...
    # Проверяем тот ли это товар
    current_item_name = cart_item.locator(
        '.inventory_item_name'
    ).text_content()
    item = data['item_name']
    assert current_item_name == item, WRONG_ITEM.format(
        current_item_name,
        item
    )
...
def test_test_checkout_step_one(browser: Browser):
    ...
    expect(
        page.locator('.checkout_summary_container'), FORM_FALUED
    ).to_be_visible()
...
def test_checkout_complete(browser: Browser):
    ...
    expect(
        page.locator('.inventory_item').nth(0), ELEMENTS_NOT_EXIST
    ).to_be_visible()
    # проверяем что товаров в корзине нет
    expect(
        page.locator('.shopping_cart_badge'), CART_NOT_EMPTY
    ).not_to_be_visible()
